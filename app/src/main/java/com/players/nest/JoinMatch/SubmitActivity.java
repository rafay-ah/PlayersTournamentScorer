...
public class SubmitActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        preferences = getSharedPreferences(SHARED_PREF+Objects.requireNonNull(matchDetail.getMatch_ID()), MODE_PRIVATE);
    }
    ...
    private void getDataFromMatchStartedActivity() {
        ...
        if (currentUser != null && opponentUser != null && matchDetail != null) {
    ...
    private void attachApprovalListener() {
        ...
        valueEventListener = new ValueEventListener() {
            ...
            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                Toast.makeText(SubmitActivity.this, error.getMessage(), Toast.LENGTH_SHORT).show();
    ...
    private void matchDispute() {
        ...
        FirebaseDatabase.getInstance().getReference(getString(R.string.DB_MATCHES))
            ...
            if (task.isSuccessful()) {
                Toast.makeText(SubmitActivity.this, "Your Opponent has filed a Match Dispute.",
                        Toast.LENGTH_SHORT).show();
                Intent intent = new Intent(SubmitActivity.this, MatchDisputes.class);
                intent.putExtra(Constants.MATCH_DETAIL_OBJECT, matchDetail);
                intent.putExtra(Constants.OPPONENT_USER_OBJECT, opponentUser);
                finishAndRemoveTask();
                startActivity(intent);
            } else {
                Toast.makeText(SubmitActivity.this, Objects.requireNonNull(task.getException()).getMessage(),
                        Toast.LENGTH_SHORT).show();
            }
        });
    }
...
//                chooseWinner();
    ...
    protected void onStart() {
        ...
        scoreReference = FirebaseDatabase.getInstance().getReference(getString(R.string.DB_MATCHES))
                .child(matchDetail.getMatch_ID());
        ...
        if (win) {
            userDetails.addListenerForSingleValueEvent(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    ...
                    user.setAccount_balance(winner(0, matchDetail.getEntryFee() * 2, user.getAccount_balance()));
                        ...
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            for (DataSnapshot data : snapshot.getChildren()) {
                                ...
                                if (usr.getRole() == 1) {
                                    ...
                                    FirebaseDatabase.getInstance().getReference("users").child(data.getKey()).setValue(usr);
                                }
                            }
                        }
