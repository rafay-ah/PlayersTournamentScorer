...
public class TournamentMatchStarted extends AppCompatActivity {
    ...
    private void getOpponentData() {
        ...
        if (firebaseUser.getUid().equals(currentMatch.getUserOneId()))
            userReference = firebaseDatabase.getReference(getString(R.string.DB_USERS))
                    .child(currentMatch.getUserTwoId());
        else if(firebaseUser.getUid().equals(currentMatch.getUserTwoId()))
            userReference = firebaseDatabase.getReference(getString(R.string.DB_USERS))
                    .child(currentMatch.getUserOneId());
        else return;
        userReference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if (!oppUser.getProfilePic().equals(""))
                    Glide.with(TournamentMatchStarted.this).load(oppUser.getProfilePic()).into(oppProPic);
    ...
    private void getCurrentUserData() {
        ...
        gameName.setText(tournamentDetail.getGame().getName());
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if( currentUser != null) {
                    ...
                    if (!currentUser.getProfilePic().equals(""))
                        Glide.with(TournamentMatchStarted.this).load(currentUser.getProfilePic()).into(userProPic);
                }
    ...
    private void updateDatabase() {
        ...
        currentMatch.setTimerFinished(true);
        ...
        if(currentMatchPosition != -1) {
            FirebaseDatabase.getInstance().getReference("Tournament")
                    ...
                    .child("matchDetails")
                    .child(String.valueOf(currentMatchPosition))
            ...
            FirebaseDatabase.getInstance().getReference("Tournament")
                    .child(tournamentDetail.getTournamentID())
                    .child("matchDetails")
                    .child(currentMatch.getMatch_ID())
                    .setValue(currentMatch);
        }
    ...
    private void openSubmitActivity() {
        ...
        // TODO change submit result accordingly
        ...
        Intent intent = new Intent(TournamentMatchStarted.this, TournamentSubmitActivity.class);
        intent.putExtra(Constants.USER_OBJECT, user);
        intent.putExtra("TOURNAMENT_DETAILS", tournamentDetail);
        intent.putExtra("CURRENT_MATCH",currentMatch);
    ...
    private void RejectRequest() {
        ...
        //TODO match status started
        matchReference.updateChildren(matchStatusMap);
    }

    public void Alert(String title, String msg, String approve, String cancel, TournamentMatches matchDetail) {
        isOpen = false;
        if (!isCancel) {
            dialog.setTitle(title)
                    ...
                    .setPositiveButton(approve, new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
//                            Intent a = new Intent(TournamentMatchStarted.this, MainActivity.class);
...
//                            startActivity(a);
    ...
    private void addScoresToDatabase(String score1, String score2) {
        ...
        firebaseDatabase.getReference("Tournament").child(tournamentDetail.getTournamentID())
                .child("matchDetails")
                .child(currentMatch.getMatch_ID())
    ...
    private void addListener(){
        databaseReferenceListener = FirebaseDatabase.getInstance().getReference("Tournament")
                .child(tournamentDetail.getTournamentID())
                .child("matchDetails")
                .child(currentMatch.getMatch_ID())
                .child(getString(R.string.DB_MATCHES_SCORES))
    ...
    private void attachScoresListener() {
        ...
        valueEventListener = new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                TournamentMatches matchDetail2 = snapshot.getValue(TournamentMatches.class);
                ...
                if (!matchDetail2.isTimerFinished()) {
                    for (DataSnapshot dataSnapshot : snapshot.getChildren()) {
                        ...
                        if (matchDetail2.getMatchStatus().equals(Constants.APPROVE_CANCEL)) {
                            ...
                            if (isEnd) {
                                isEnd = false;
                                Toast.makeText(TournamentMatchStarted.this, "Your Match Has Been Canceled", Toast.LENGTH_SHORT).show();
                                Intent a = new Intent(TournamentMatchStarted.this, MainActivity.class);
                                ...
                                user.setAccount_balance(user.getAccount_balance() + tournamentDetail.getEntryFee());
                                FirebaseDatabase.getInstance().getReference(getString(R.string.DB_USERS))
                    ...
                    matchReference.removeEventListener(valueEventListener);
                    countDownTimer.cancel();
                    Toast.makeText(TournamentMatchStarted.this, "Timer Finished", Toast.LENGTH_SHORT).show();
                    openSubmitActivity();
                }
            }
    ...
    private void notifyOpponent() {
        ...
        new NotificationHelper().sendNotification(getApplicationContext(), opponentUser.getDeviceToken(), tournamentDetail.getGame().getName(),
    ...
    protected void onStart() {
        ...
        if(currentMatchPosition != -1) {
            matchReference = firebaseDatabase.getReference("Tournament").child(tournamentDetail.getTournamentID())
            .child("matchDetails").child(String.valueOf(currentMatchPosition));
            matchReference.addValueEventListener(valueEventListener);
        }
        else if(currentMatch != null){
            matchReference = firebaseDatabase.getReference("Tournament").child(tournamentDetail.getTournamentID())
                    .child("matchDetails").child(currentMatch.getMatch_ID());
            matchReference.addValueEventListener(valueEventListener);
        }
        updateClockTxt();
        startTimer();
